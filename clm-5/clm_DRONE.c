/* translate drone in naive.lisp to C
 *   written Thu 6-Oct-22 at 2:31 by CLM of 8-May-18
 */
...
int clm_drone2 (double *clm_double, int datar_len, int *clm_int, int datai_len)
{
  /* The _clm_* variables are temporary values generated by the run macro */
  void (*old_SIGINT)(int);
  mus_long_t I;
  double _clm_5010, _clm_5011, _clm_5012, _clm_5013, _clm_5014;
  mus_any *LOC = NULL, *AMP_ENV = NULL, *S = NULL, *RAN_VIB = NULL;
  void *all_gens = NULL;
  ...
  if (CLM_RAND_P(5))
    RAN_VIB = clm_add_gen_to_genbag(all_gens, 
      mus_make_rand_with_distribution(
        clm_double[CLM_RLOC(5)],
        clm_double[CLM_RLOC(5) + 1],
        (clm_int[CLM_ILOC(5) + 2] > 0) ? ((double *)(clm_double + CLM_RLOC(5) + 2)) : NULL,
        clm_int[CLM_ILOC(5) + 2]));
  if (CLM_TABLE_LOOKUP_P(7))
    S = clm_add_gen_to_genbag(all_gens, 
      mus_make_table_lookup(
        clm_double[CLM_RLOC(7)],
        clm_double[CLM_RLOC(7) + 1],
        (double *)(clm_double + CLM_RLOC(7) + 2),
        clm_int[CLM_ILOC(7) + 2],/* size */
        clm_int[CLM_ILOC(7) + 3])); /* type */
  if (CLM_ENV_P(9))
...
SAMPLE_LOOP_BEGIN:
  ...
  if (I > clm_end) {I = clm_end; goto RUN_ALL_DONE;}
                                                                                /* (progn */
  _clm_5012 = mus_env(AMP_ENV);                                                 /* (env AMP-ENV) */
  _clm_5014 = mus_rand(RAN_VIB, 0.0);
  _clm_5013 = mus_table_lookup(S, _clm_5014);
                                                                                /* (* _clm_5012 _clm_5013) */
  _clm_5011 = ((_clm_5012) * (_clm_5013));
